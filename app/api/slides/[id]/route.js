...
export async function GET(_request, { params }) {
  const item = db.prepare("SELECT * FROM slides WHERE id = ?").get(params.id);
...
export async function PUT(request, { params }) {
  ...
  const updated = db.prepare("SELECT * FROM slides WHERE id = ?").get(params.id);
  if (!updated) return NextResponse.json({ error: "Topilmadi" }, { status: 404 });
  return NextResponse.json(updated);
}

export async function DELETE(_request, { params }) {
  db.prepare("DELETE FROM slides WHERE id = ?").run(params.id);
