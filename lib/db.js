...
const SCHEMA = `
  ...
  -- "Men haqimda" tarjimai holi — yillar bo'yicha bosqichlar (timeline).
  ...
  -- Har bir safarning ichki galereyasi: bir nechta rasm + har biriga izoh.
  ...
  -- Bir martalik migratsiyalar / kalit-qiymat sozlamalari uchun.
  CREATE TABLE IF NOT EXISTS app_meta (
    key TEXT PRIMARY KEY,
    value TEXT DEFAULT ''
  );

  -- Foydalanuvchilar yuborgan xato-xabarlari (admin panelda ko'rinadi).
...
const SOCIAL_PROFILE_DEFAULTS = [
  ["instagram", "Otabek Sobitov", "@otabek.yuldashevich.sobitov", "https://www.instagram.com/otabek.yuldashevich.sobitov/"],
  ...
  [
    ...
    "Otabek Sobitov",
    "otabek-sobitov",
    "https://www.linkedin.com/in/otabek-sobitov-88257338b?originalSubdomain=uz",
  ],
];
...
const LATE_COLUMNS = [
  ...
  ["travels", "body", "TEXT DEFAULT ''"],
  ["about", "contact_email", "TEXT DEFAULT ''"],
  ["about", "contact_phone", "TEXT DEFAULT ''"],
  ["about", "contact_address", "TEXT DEFAULT ''"],
  // Facebook Sahifasi (Page) havolasi — to'ldirilsa karta jonli lentaga aylanadi.
  ["social_profiles", "page_url", "TEXT DEFAULT ''"],
  // Instagram uslubidagi karta uchun: bio matni va "following" (kuzatilayotganlar) soni.
  ["social_profiles", "bio", "TEXT DEFAULT ''"],
  ["social_profiles", "following", "TEXT DEFAULT ''"],
  // LinkedIn kartasi uchun: joylashuv (masalan "Toshkent, O'zbekiston").
  ["social_profiles", "location", "TEXT DEFAULT ''"],
  // Instagram: post sonini qo'lda kiritish uchun (bo'sh bo'lsa — qo'shilgan postlar soni).
  ["social_profiles", "posts_count", "TEXT DEFAULT ''"],
];

// Berilgan kalit bo'yicha migratsiyani faqat bir marta bajaradi.
async function runOnce(key, fn) {
  const done = await client.execute({
    sql: "SELECT 1 FROM app_meta WHERE key = ?",
    args: [key],
  });
...
async function initSchema() {
  ...
  // Admin parolini env orqali qayta o'rnatish (parolni unutgan bo'lsa).
  // ADMIN_RESET_PASSWORD Vercel env'ga qo'yilsa, sayt uni bir marta yangi
  // parol qilib o'rnatadi. Kalit parol qiymatiga bog'langan — bir xil qiymat
  // qayta ishlamaydi, faqat qiymat o'zgarsa yana ishlaydi (in-app o'zgartirishni
  // buzmaydi). Login'da foydalanuvchi nomi shart emas — yagona admin qatori
  // bo'yicha tekshiriladi.
  const resetPw = process.env.ADMIN_RESET_PASSWORD;
  if (resetPw && resetPw.length >= 8) {
    const key =
      "pwreset:" +
      crypto.createHash("sha256").update(resetPw).digest("hex").slice(0, 24);
    await runOnce(key, async () => {
      const hash = bcrypt.hashSync(resetPw, 12);
  ...
  // Bir martalik: "Men haqimda" portretini yangi tiniq rasmga o'rnatamiz.
  // Flag qo'yilgach qayta ishlamaydi — admin keyin xohlagancha almashtiradi.
  await runOnce("about_photo_v2", async () => {
    await client.execute({
      sql: "UPDATE about SET photo_url = ? WHERE id = 1",
      args: ["/about/otabek-sobitov.png"],
    });
  });
  ...
  // Bir martalik: LinkedIn profil havolasini yangi manzilga o'rnatamiz.
  // Flag qo'yilgach qayta ishlamaydi — admin keyin xohlagancha almashtiradi.
  await runOnce("linkedin_url_v2", async () => {
    await client.execute({
      sql: "UPDATE social_profiles SET profile_url = ? WHERE network = 'linkedin'",
      args: ["https://www.linkedin.com/in/otabek-sobitov-88257338b?originalSubdomain=uz"],
    });
  });
}
